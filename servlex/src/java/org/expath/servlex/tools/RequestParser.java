...
{
    public RequestParser(HttpServletRequest request, String path, String appname, Processors procs)
    {
        myRequest = request;
        myPath    = path;
        myAppName = appname;
     ...
     * components)
     */
    public Document parse(TreeBuilder b, List<Item> input, boolean trace_content)
            ...
            throws TechnicalException
    {
        ...
        if ( myMatcher == null || myServlet == null ) { // -> welcome file or resource
            ...
            while ( (part = myMatcher.next()) != null ) {
                if ( myMatcher.isGroup() && myMatcher.groupNumber() > 0 ) {
                    b.startElem("match");
